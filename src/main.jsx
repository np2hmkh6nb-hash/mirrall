...
const posters = [
  ['cover.webp', '封面图.jpg'], ['poster-chuge.webp', '楚歌.jpg'], ['poster-battlefield.webp', '古战场.jpg'], ['poster-shadow.webp', '心魔.jpg'],
  ['poster-token.webp', '信物.jpg'], ['poster-fight.webp', '战斗.jpg'], ['poster-mirror.webp', '破镜.jpg'], ['poster-letter.webp', '写信.jpg'],
];
const clips = [
  ['kling_20260501_作品_镜头_1_2s_大全_3482_0.mp4', '梦境启幕', 'cover.webp'],
  ['kling_20260501_作品_镜头_1_2s_近景_3757_0.mp4', '近景凝视', 'poster-shadow.webp'],
  ['kling_20260501_作品_定格___卷轴收束__3908_0.mp4', '卷轴收束', 'poster-ending.webp'],
  ['kling_20260501_作品_镜头_1_2s_快切_3857_0.mp4', '楚歌快切', 'poster-fight.webp'],
];
...
function VideoPreview({ src, title, poster }) {
  const [active, setActive] = useState(false);
  return <article className="workCard" onMouseEnter={() => setActive(true)} onMouseLeave={() => setActive(false)} onClick={() => setActive(true)}>
    {active ? <video autoPlay muted loop playsInline preload="metadata" poster={`${rootPath}${poster}`}><source src={`${rootPath}${src}`} type="video/mp4" /></video> : <img className="videoPoster" src={`${rootPath}${poster}`} alt={`${title} 视频预览`} loading="lazy" decoding="async" />}
    <div><span>AI VIDEO</span><h3>{title}</h3></div>
  </article>;
}

function CampaignPlayer() {
  const [ready, setReady] = useState(false);
  return <div className="campaignVideo">
    <video key={ready ? 'ready' : 'idle'} controls={ready} autoPlay={ready} preload="none" poster={`${rootPath}5月1日-封面.jpg`}>
      {ready && <source src={`${rootPath}5月1日-web.mp4`} type="video/mp4" />}
    </video>
    {!ready && <button className="campaignPlay" onClick={() => setReady(true)} aria-label="播放完整营销案例">▶</button>}
  </div>;
}
...
function App() {
  ...
  return <>
    ...
    <main>
      <section id="top" className="hero"><video className="heroVideo" autoPlay muted loop playsInline preload="metadata" poster={`${rootPath}cover.webp`}><source src={`${rootPath}hero-loop.mp4`} type="video/mp4" /></video><div className="heroShade" /><div className="heroInner"><p className="eyebrow">AI DESIGNER / PRODUCT THINKER</p><h1>Zoe's<br /><span>数字幻境</span></h1><p className="heroCopy">我是 Zoe，一个还在不断试验的 AI 产品探索者。喜欢把新技术拆开、重组，再做成有趣、好用，也有一点想象力的体验。</p><a className="scrollCue" href="#project">向下探索 <b>↓</b></a></div><div className="heroStamp">Z / 2026<br />SCROLL TO ENTER</div></section>
